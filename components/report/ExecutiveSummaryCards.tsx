...
export default function ExecutiveSummaryCards({ analysis, onUpdate, sectionConfig }: Props) {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* ── 상위권 vs 중하위권 ── */}
      {!isHidden("exec_level_comparison") && <div className="space-y-4">
        <EditableText
      ...
      {!isHidden("exec_insights") && (
        <div className="space-y-4">
          ...
          {Array.from({ length: insightCount }, (_, i) => {
            const DEFAULTS = [
              { color: "#F97316", title: "체감 난이도 상승" },
              { color: "#0B1F4D", title: "압도적 단원 편중" },
              { color: "#DC2626", title: "고득점의 열쇠 = 학교 프린트" },
            ];
            const color = DEFAULTS[i]?.color ?? "#6B7280";
            const titleKey = `exec_insight_title_${i}`;
            return (
